console.log('Alhadath build 1756253301');
// Storefront logic with Telegram checkout (via GET open in new tab)
// Cart behavior: add-to-cart updates badge only (no auto open)

const STORE_KEY = "alhadath_cart_v2";
const state = {
  products: [],
  filtered: [],
  brand: "all",
...
const els = {
  ...
  addr: document.getElementById("cust-addr"),
...
function applyFilters(){
  ...
  state.filtered = state.products.filter(p => {
    const brandOk = state.brand === "all" ? true : (state.brand === "promo" ? (p.promo === true) : p.brand === state.brand);
...
function initCartModal(){
  ...
  els.confirmOrder.addEventListener("click", sendOrderToTelegram);
}
...
function buildOrderMessage(){
  const items = Cart.load();
  const lines = items.map(i => `${i.name} x${i.qty} = ${i.price * i.qty} JD`).join("\n");
  const total = Cart.total();
  const info = [
    `الاسم: ${els.name.value || '-'}`,
    `الهاتف: ${els.phone.value || '-'}`,
    `العنوان: ${els.addr.value || '-'}`,
    `ملاحظات: ${els.notes.value || '-'}`
  ].join("\n");
  const msg = `طلب جديد من المتجر:\n${lines}\n---\nالإجمالي: ${total} JD\n${info}`;
  return msg;
}

function sendOrderToTelegram(){
  if(!window.TG || !window.TG.token || !window.TG.chat_id || window.TG.token.includes("PUT_")){
    alert("⚠️ الرجاء ضبط TELEGRAM TOKEN و CHAT_ID داخل ملف config.js قبل الإرسال.");
    return;
  }
  const msg = buildOrderMessage();
  const url = `https://api.telegram.org/bot${encodeURIComponent(window.TG.token)}/sendMessage?chat_id=${encodeURIComponent(window.TG.chat_id)}&text=${encodeURIComponent(msg)}`;
  // فتح في تبويب جديد لتجاوز قيود CORS، وسيظهر رد JSON من تيليجرام
